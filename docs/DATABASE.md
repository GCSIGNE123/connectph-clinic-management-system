...
Migration `0023_sync_jobs.py` (`down_revision = "0022_pre_queue_vitals"`). Two new tables, no changes to any existing table.

- **`sync_jobs`**: `entity_type` (`String(50)`), `record_id` (UUID, the id of the source-of-truth row in `patients`/`visits`/etc.), `operation` (`create`/`update`/`delete`), `payload` (`JSONB`, a full snapshot of the record at enqueue time), `status` (`pending`/`in_progress`/`completed`/`failed`), `retry_count` (`Integer`, default 0), `next_retry_at` (nullable `DateTime`), `last_error` (nullable `String(2000)`), `completed_at` (nullable `DateTime`), `duration_ms` (nullable `Integer`), plus `TenantMixin`'s `clinic_id` and `TimestampMixin`'s `created_at`/`updated_at`. This is the persistent local→cloud upload queue drained by `backend/app/services/sync_worker_service.py`; a row is never hard-deleted or dropped on failure — a failed job stays `failed` with an incrementing `retry_count`/advancing `next_retry_at` until it eventually succeeds.
  - `ix_sync_jobs_status_created_at` (composite on `status, created_at`): the worker's primary query shape - oldest-pending-first.
  - `ix_sync_jobs_next_retry_at`: supports the "is this failed job's backoff window over yet" filter.
- **`synced_records`**: what a **cloud-hosted instance of this same codebase** (pointed at `CLOUD_DATABASE_URL`) stores incoming `POST /api/v1/backup/{entity_type}` uploads into. `clinic_id` (plain UUID, deliberately **no FK** to `clinics.id` - see below), `entity_type`, `record_id`, `operation`, `payload` (`JSONB`), `synced_at`, plus `TimestampMixin`'s `created_at`/`updated_at`. Upsert-keyed on `uq_synced_records_clinic_entity_record` (unique on `clinic_id, entity_type, record_id`) - a later upload for the same record simply overwrites the stored snapshot.
  - **No FK to `clinics.id`**: this table intentionally is NOT a fully-normalized relational mirror of the source database. Replaying a JSONB snapshot into a cloud-side `patients`/`visits`/... table would require the cloud database to also independently hold every foreign-keyed parent row (`clinics`, `users`, `doctors`, ...) in the correct dependency order, which is out of scope for a one-way *backup* (whose job is "have an off-site, restorable copy of the data," not "run a live relational replica"). Storing a document-style snapshot keyed by `(clinic_id, entity_type, record_id)` is a deliberate, simpler, dependency-free design for this milestone - see `backend/app/models/synced_record.py`'s docstring.
- **No `LegacyMixin`** on either table: net-new tables with no legacy-system equivalent, same rationale as Phase 15/18/20/21's new tables.
- **Both tables exist in every environment's migration history** (the same codebase/migration chain runs on both a clinic's local instance and a cloud-hosted instance) but `synced_records` is only ever written to when a given instance is actually acting as the cloud endpoint, and `sync_jobs` is only ever meaningfully drained when `DEPLOYMENT_MODE=hybrid` with a reachable `CLOUD_API_URL` (see `docs/FEATURES.md`).

**Down-revision chain**: `0023_sync_jobs` → `0022_pre_queue_vitals` (unchanged, append-only). Applied cleanly against the real dev database (`alembic upgrade head`, `0022_pre_queue_vitals` → `0023_sync_jobs`) with no data loss - purely additive new tables, no existing row touched. Also applied cleanly to a separate throwaway database used to stand in for "the cloud" during this milestone's live verification (see `docs/TESTING.md`), then that throwaway database was dropped.

## Post-RC1 — Vaccination Administration

Migration `0024_vaccination_administration.py` (`down_revision = "0023_sync_jobs"`). One new table, no changes to any existing table - mirrors `laboratory_orders`' "workflow record layered 1:1 on top of a Phase 9 `Order`" design (see Phase 10 section above).

- **`vaccination_administrations`**: `order_id` (UUID, unique FK to `orders.id`, `ondelete=CASCADE`), `branch_id`/`visit_id`/`patient_id`/`doctor_id` (denormalized copies from the order, same as `laboratory_orders`), `vaccine_name` (`String(255)`, copied from the order's first item at creation, editable at administration time in case the actual vaccine given differs), `status` (`vaccination_status` enum: `Requested`/`Administered`/`Cancelled` - deliberately simpler than Laboratory's five-state machine, since a vaccination either has been given or it hasn't), `dose`/`lot_number`/`site`/`route` (all nullable `String(100)`, populated only at administration time), `notes` (nullable `Text`), `administered_at` (nullable `DateTime`), `administered_by` (nullable FK to `users.id`, `ondelete=SET NULL`). Carries `TenantMixin`, `SoftDeleteMixin`, `TimestampMixin`, and `LegacyMixin` fields, consistent with `laboratory_orders`.
- **Auto-created**: `ClinicalOrdersService.create_order()` calls `VaccinationService.create_from_order()` whenever `order_category == "Vaccination"`, exactly mirroring the existing Laboratory auto-attach — a doctor ordering a vaccination via the unchanged Phase 9 `POST /consultations/{id}/orders` gets a matching `vaccination_administrations` row with no extra step.
- **Who may administer is intentionally broader than other order categories**: `VACCINATION_ADMINISTER_ROLES = {Owner, Administrator, Doctor, Nurse, Receptionist}` in `core/dependencies.py` — unlike Laboratory (Laboratory role only) or general clinical-orders edit (assigned doctor only), any Nurse or Receptionist in the clinic may administer a vaccination a doctor ordered, not just the ordering doctor. This was an explicit product decision (client confirmed: Nurse role + Receptionist role, both) since real clinic staffing routinely has front-desk/nursing staff give injections a doctor ordered.
- **`administer()` also advances the parent `Order`** to `Completed` (and `cancel()` to `Cancelled`), keeping the Phase 9 Order's own status in sync everywhere it's read (Doctor Workspace's Clinical Orders tab, visit timeline) — same "keep the parent Order in sync" requirement Laboratory follows.
- **No `LegacyMixin` reuse beyond the standard fields**: net-new table with no legacy-system equivalent.

**Bug found and fixed during build**: `TenantUserAdminService._get_user()` (platform-admin tenant-user management, unrelated to this table but touched in the same session) was missing `selectinload(User.role)`, causing a 500 on a fresh DB session whenever the response needed `user.role.name` — it happened to pass in tests only because of stale SQLAlchemy identity-map caching from an earlier query in the same test. Fixed by eager-loading the relationship (see `docs/BUGS.md` BUG-032).

**Down-revision chain**: `0024_vaccination_administration` → `0023_sync_jobs` (unchanged, append-only). Applied cleanly against the real dev database with no data loss - purely additive new table, no existing row touched. Live-verified end-to-end: a Doctor ordered a vaccination via the real API, the `vaccination_administrations` row auto-created correctly, a real Nurse account (distinct from the ordering Doctor) administered it via the browser UI, the underlying Order mirrored to `Completed`, and a Cashier-role account was correctly rejected with 403 when attempting the same action.
