...
| BUG-042 | `test_consultations.py::test_complete_consultation_reflects_onto_visit_status` and `::test_sign_consultation` are stale: `complete_consultation` now goes straight to Signed (by design, see the service comment) but the tests still expect Completed and then a separate sign call. Found while running the existing consultation tests for Task #2; unrelated to it and left unchanged. | Low | Open | Tests | - |
| BUG-044 | Pre-existing, found (not caused) during Task #6 live acceptance: two `consultations` rows can be created for the same visit a few milliseconds apart. **Symptom**: on the Pilot DEV visit `VIS-20260811-000025` the Nurse opened Queue -> Enter Vitals once and the visit ended up with two consultations, `5f1af60d…` (`Draft`, created 07:21:54.510, no SOAP note) and `662dc7ed…` (`InProgress`, created 07:21:54.520, holds the SOAP note) - about 10 ms apart; the browser network log showed the two matching `GET .../soap/subjective-objective` reads. **Second occurrence**: the Task #6 pre-queue test visit `VIS-20260921-000001` (Demo) also got two consultations, `d4ccbeae-...` (`Draft`) and `8cfa1488-...` (`InProgress`, holds the SOAP note), created 5 ms apart via the New Queue -> Enter Vitals step; that test record has since been removed. **Likely cause**: a check-then-create race in `ConsultationService.open_consultation` (reached via `POST /visits/{id}/consultation/open-for-reception` and the Doctor's `.../consultation/open`) - it looks up an existing consultation for the visit and creates one if none is found, and `consultations` has no unique constraint on the visit (see the note in `models/consultation.py`), so two overlapping calls each see "none" and each insert. **Possible trigger (not confirmed)**: two overlapping open calls from the same screen, e.g. the Enter Vitals dialog's open effect running twice in development; this was not proven. **Affected code path**: `open_consultation` does `repo.get_latest_for_visit(...)` then `repo.create_consultation(...)` with no lock (`consultation_service.py`); `models/consultation.py` documents that a DB-level unique constraint was deliberately not used and that readers resolve "the" consultation as latest-created-wins. **Risk**: because latest-wins, readers normally pick the later row (here the one holding the SOAP note), but the earlier `Draft` is left as an orphan duplicate, and if the two rows' order ever differs from the order data was written to them, a reader could pick the row without the S/O values. **Not introduced by Task #6**: `consultations.py`, `open_consultation` and the dialog's open effect are unchanged by it (the only change to `consultation_service.py` is removing two field names from the S/O allowlist). **No remediation performed**: no code change, no data cleanup, nothing touched in production - DEV observation only. Suggested follow-up: make open idempotent (unique partial index on the active consultation per visit and/or `INSERT ... ON CONFLICT`/row lock) and add a concurrent-open test. | Medium | Open | backend | unassigned |

## Resolved Bugs

| ID | Title | Severity | Resolved | PR/Commit |
|---|---|---|---|---|
| BUG-043 | **Null-padding of unspecified SOAP fields wiped stored values (Task #6).** `fromSoapNoteInput` in `frontend/src/features/consultation/api/consultation-api.ts` serialized EVERY SOAP field the caller did not supply as an explicit `null` (a payload with only `chiefComplaint` went out with ~24 other keys set to `null`). The backend merge-save (`save_soap` / `save_soap_subjective_objective`, which write only the keys present via `model_dump(exclude_unset=True)`) treats an explicit `null` as a real key, so each null OVERWROTE the stored value. **Affected**: (1) every Doctor autosave / Save Progress on the consultation page (the autosave hook sent the whole form), so a value entered by a Receptionist/Nurse or by another tab could be wiped by a Doctor autosave that had loaded an older snapshot; (2) every Reception/Nurse save from Enter Vitals and the pre-queue step, where saving one vital nulled the other vitals and Subjective fields. **Fix**: `fromSoapNoteInput` now includes only keys whose value is not `undefined` (an explicit `null`/empty is sent only when the user intentionally clears a field); `saveSubjectiveObjective` picks only allowlisted supplied keys; the Doctor autosave hook (`use-soap-autosave.ts`) keeps a baseline and sends only fields changed since the last load/successful save (`changedSoapFields`), discarding a slower in-flight response for a previous consultation; `ReceptionVitalsDialog` and `PreQueueVitalsStep` send only changed fields. Server side, the reception S/O endpoint now rejects unsupported keys with 422 (`extra="forbid"`), so a client can no longer pad or smuggle Doctor-only keys. **Regression tests**: `consultation-api.payload.test.ts` (5, incl. no-null-padding), `use-soap-autosave.test.tsx` (15, incl. changed-fields-only, baseline after save, stale-consultation response ignored), `ReceptionVitalsDialog.test.tsx` (21) and `PreQueueVitalsStep.test.tsx` (8) assert exact payloads; `backend/app/tests/test_reception_soap.py` (61) covers merge integrity, 422 on unknown/Doctor-only keys and role gates. Live DEV: a Doctor Save Progress sent exactly `{"physical_examination":"..."}`, and the 30 s timer autosave sent exactly `{"physical_examination":""}`; Receptionist-entered values, vitals and pain score stayed intact. Manual two-session race reproduction was not practical; dedicated autosave tests cover the race scenario. | High | 2026-09-21 | `frontend/src/features/consultation/api/consultation-api.ts`, `frontend/src/features/consultation/hooks/use-soap-autosave.ts`, `frontend/src/features/consultation/components/intake-fields.tsx`, `ReceptionVitalsDialog.tsx`, `frontend/src/features/queue/components/PreQueueVitalsStep.tsx`, `backend/app/schemas/consultation.py`, `backend/app/services/consultation_service.py`, `backend/app/api/v1/soap_suggestions.py` (see Task #6 in FEATURES.md). No migration. |
| BUG-041 | Doctor Workspace's Clinical Orders "Orders" tab had a single free-text "Item / test name" field and one "Create Order" button (`ClinicalOrdersTab.tsx` hardcoded `items: [item]`) — after BUG-040 fixed the backend to accept multiple `OrderItem`s per Order, the actual Doctor UI still had no way to submit more than one lab test in a single request; a doctor still had to click "Create Order" once per test. Not a backend defect (the API already fully supported `items: list[...]`), a UI gap closed as Task #3. | Medium | 2026-09-18 | `frontend/src/features/clinical-orders/components/ClinicalOrdersTab.tsx` (Laboratory category only: replaced the free-text field with a searchable multi-select sourced from `GET /laboratory/templates` via the existing `useLaboratoryTemplates` hook — reusing `components/ui/searchable-select.tsx` plus a selected-items chip list; one submit now sends every selected test as one `items` array in a single Order; Radiology/Vaccination/Custom untouched), `frontend/src/features/clinical-orders/components/ClinicalOrdersTab.test.tsx` (12 new tests). No backend changes. |
| BUG-040 | `ClinicalOrdersService.create_order()` (Doctor Workspace → Clinical Orders tab) only ever converted `order.items[0]` into a `laboratory_orders` row — a Laboratory Order submitted with multiple `OrderItem`s (e.g. CBC + Urinalysis + FBS in one submission, already fully supported end-to-end by `OrderCreate.items: list[...]`) silently dropped every item after the first: it never reached the Laboratory worklist, never got a lifecycle, and never got billed. The invoice-total aggregation itself was never wrong — it only ever saw the one `LaboratoryOrder` that existed. Reported by the clinic as "billing total wrong for multiple lab requests." Root cause: `LaboratoryService.create_from_order` read `order.items[0].item_name` instead of iterating `order.items`, and `laboratory_orders.order_id` carried a UNIQUE constraint (migration `0011`) enforcing a strict 1:1 `Order`↔`LaboratoryOrder` relationship that made a second row for the same Order structurally impossible even if the code had tried. Confirmed live: all 271 pre-existing `laboratory_orders` rows in the Dev database predate this fix and have no way to distinguish which `OrderItem` (if any) they came from. | High | 2026-09-18 | `backend/alembic/versions/0044_laboratory_order_item_fk.py` (additive migration: drops `order_id`'s UNIQUE constraint, keeping its FK/index; adds nullable `order_item_id` UUID FK to `order_items.id` with its own unique index — applied to Dev DB, verified all 271 existing rows keep `order_item_id = NULL` and remain readable), `backend/app/models/laboratory_order.py` (`order_item_id` column + `order_item` relationship), `backend/app/repositories/laboratory_repository.py` (`get_by_order_item_id` replaces the old `get_by_order_id`, which would have raised `MultipleResultsFound` once an Order legitimately owns more than one `LaboratoryOrder`; eager-loads `order_item`), `backend/app/services/laboratory_service.py` (`create_from_order` now loops over every `order.items` entry, creating one `LaboratoryOrder` each, idempotent per `OrderItem`; `LaboratoryOrderRead` gained `order_item_id`), `backend/app/schemas/laboratory.py` (`order_item_id` field), `backend/app/services/clinical_orders_service.py` (`create_from_order` return type is now `list[LaboratoryOrder]`; the Cloud Backup sync-enqueue loop binds each iteration's `lab_order` via a `lambda lo=lab_order: ...` default argument to avoid a late-binding closure bug now that it's a loop instead of a single call — still inside the same single `session.commit()` transaction as the parent `Order`, per BUG-038's atomicity fix, unchanged). No change was needed to `_sync_billing` (already per-`LaboratoryOrder` and idempotent) or to invoice-total aggregation (already a sum over `invoice.items`, unaffected by how many `LaboratoryOrder` rows exist). Tests: `backend/app/tests/test_clinical_orders.py` (3 new: single-item unchanged, 2-item and 3-item fan-out with correct `order_item_id` mapping) and `backend/app/tests/test_laboratory.py` (3 new: CBC ₱200 + Urinalysis ₱100 + FBS ₱80 = ₱380 across 3 fanned-out `LaboratoryOrder`s with no duplicate invoice line items, partial completion of a multi-item order, and a directly-constructed pre-migration-style `LaboratoryOrder` row with `order_item_id = NULL` confirmed still readable via the API). Full regression: `test_clinical_orders.py` 20/20, `test_laboratory.py` 165/165, `test_billing.py` 16/16, all passing. |
| BUG-039 | `QueueService.get_slip()`'s Laboratory vitals exemption checked `department.department_code == "LAB"` directly, which only matches the *seeded* default Laboratory department — a clinic with its own manually-created Laboratory department (a real, observed case: Canora Medical Clinic's Laboratory department has code `D03`) was not exempted, so a Laboratory Pay-First queue ticket's very first print (right after payment succeeded) 400'd with "Vital signs must be taken before printing the queue ticket." even though Laboratory tickets have no consultation/SOAP note to carry vitals in at all. Root cause: the Pay-First workflow introduced a second, correct, NAME-based Laboratory-detection helper (`_is_laboratory_department`, used by queue creation's payment gate) but never updated `get_slip` to match — the two checks silently drifted apart. Confirmed live against the dev database (real clinic's department_code is `D03`, not `LAB`) and reproduced/verified fixed in the browser (`GET .../slip` 400 → 200 after the fix, `vitals_taken: false`, `is_paid: true`). | High | 2026-08-19 | `backend/app/services/queue_service.py` (`get_slip`: reuse the existing shared `_is_laboratory_department()` helper instead of the narrower `department_code == "LAB"` check), `backend/app/tests/test_laboratory_payment_first_queue.py` (new regression test `test_lab_payment_first_queue_slip_with_custom_department_code`, using a deliberately non-"LAB" department code; verified to genuinely fail against the pre-fix code via `git stash`) — commit `f002188` |
